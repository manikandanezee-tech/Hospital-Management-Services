...
@RestController
@RequestMapping("/{authToken}/billing")
public class BillingController {
	...
	public BillingIO save(@PathVariable String authToken, @RequestBody BillingIO billingIO) throws SQLException {

		AuthDTO authDTO = userService.getAuthDTO(authToken);

		NamespaceDTO namespaceDTO = namespaceService.getByCode(authDTO, billingIO.getNamespaceIO().getCode());
		AppointmentDTO appointmentDTO = appointmentService.getByCode(billingIO.getAppointmentResponseIO().getCode(), authDTO);
		...
		BillingDTO billingDTO2 = billingService.save(billingDTO, authDTO);
	...
	public BillingIO getByCode( @PathVariable String authToken, @PathVariable String code) throws SQLException {

		AuthDTO authDTO = userService.getAuthDTO(authToken);

		LOGGER.info("GET-BY-CODE - Request to get Billing data by code : {}", code);
		BillingDTO billingDTO = billingService.getByCode(code, authDTO);
